...
const 내용열쇠 = ['제목', '요약', '문단', '목록', '번호목록', '표', '단계띠', '수치', '그림', '출처', '여백'];
...
function 내용읽기(자리, i, P) {
  if (자리.내용 != null) {
    ...
    const 옛것 = 내용열쇠.filter((k) => k !== '여백' && 자리[k] != null);
  ...
  for (const 열쇠 of ['목록', '번호목록', '표', '단계띠', '수치', '그림']) {
...
/* ─────────────────── 확정 높이 ───────────────────
   ...
   불확정 문단 · 목록 · 번호목록 · 단계띠 — **줄 수가 글자에 달렸다.**
  ...
  switch (열쇠) {
    ...
    case '여백': {
      ...
      if (!Number.isInteger(n) || n < 1 || n > 30) throw new Error(
        `자리 ${i} · 내용 ${j} 의 "여백" 은 1 ~ 30 사이 블록 수다 · 한 블록이 42px 이다 ` +
...
function 블록(자리, r, i, 여백문서) {
  ...
  if (자리.비움) {
    ...
    const 있는것 = [...내용열쇠.filter((k) => k !== '여백'), '내용', '도형']
      .filter((k) => 자리[k] != null);
