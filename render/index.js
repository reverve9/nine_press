...
function 표그리기(자리, i, 안폭, 안높이, 앞높이, 뒷높이, P) {
  ...
  if (t.높이) {
    const r = 높이나누기(t.높이, 칸수, 안높이 - 앞높이 - 뒷높이, i);
    위로 = r.위로;
    let y = 0;
  ...
  const 머리뒤 = 앞높이 ? 42 : 0;
  return `<div class="tb" style="height:${총높이}px` +
...
function 그림그리기(자리, i, 안높이, 앞높이, 뒷높이) {
  const g = typeof 자리.그림 === 'string' ? { 경로: 자리.그림 } : 자리.그림;
  ...
  const 블록수 = 그림블록(g.높이, Math.floor((안높이 - 앞높이 - 뒷높이) / 42), i);
  const h = 블록수 * 42;

  /* **인라인 margin-top 을 안 쓴다** · §N-배경 b 가 표에서 물린 함정이다.
     여기서 위 간격을 적으면 CSS 의 위계 간격(`.bt + .im` 머리뒤 42)이 통째로 덮여
     그림이 42px 올라앉는다. 위 간격은 rules/page.css ㉜ 하나가 맡는다. */
  return `<div class="im" style="height:${h}px">` +
    `<img src="${경로}" alt="${esc(g.설명 ?? '')}" style="object-fit:${맞춤이름[맞춤]}">` +
    `</div>`;
}

/* ─────────────────── 블록 ───────────────────
   제목 · 요약 · 문단 · 목록 · 표 · 단계띠 · 수치 · 그림 · 출처. */
...
function 블록(자리, r, i, 여백문서) {
  ...
  const 문단배열 = Array.isArray(자리.문단);
  const 문단 = 자리.문단 == null ? [] : 문단배열 ? 자리.문단 : [자리.문단];
  문단.forEach((t, j) => {
    if (빔(t)) return;
    o.push(`<div class="bd"${dp(문단배열 ? [...P, '문단', j] : [...P, '문단'])}>${inline(t)}</div>`);
  });
  ...
  /* 표 — 「높이」를 계산하려면 앞에 놓인 것들의 높이를 알아야 한다.
     제목 · 요약 · 머리뒤는 42 로 확정이지만 문단 · 목록은 줄 수가 글자에 달렸다.
     그래서 「높이」는 **앞이 제목 · 요약뿐일 때만** 받는다. 나머지는 오류로 막는다 —
  ...
  if (자리.표) {
    ...
    /* 뒤에 오는 것도 자리를 먹는다 — 출처(.lb)는 28 + 위아래 7 = 42 · 앞 간격 0.
       이걸 안 빼면 표가 자리를 꽉 채우고 출처가 밖으로 밀린다 ·
    ...
    if (자리.표.높이) {
      ...
      if (막는것.length) throw new Error(
        `자리 ${i} 의 표가 "높이" 를 쓰는데 앞에 ${막는것.join(' · ')} 가 있다. ` +
        `줄 수를 미리 못 세서 남는 높이를 못 나눈다. "높이" 를 빼거나 제목 · 요약만 둔다`);
      // 그림도 남은 높이를 노린다 · 둘이 같은 것을 나눠 가질 수 없다 · §N-그림
      if (자리.그림 != null) throw new Error(
        `자리 ${i} 에 "높이" 를 쓰는 표와 그림이 같이 있다. 둘 다 남은 높이를 먹는 것이라 ` +
        `한 자리에 못 둔다. 표 "높이" 를 빼거나 그림을 다른 자리로 옮긴다`);
    }
  ...
  /* 그림 — 큰 덩이 가운데 마지막이다. 그 뒤는 출처(캡션 · 42) 하나뿐이라
     「채움」이 빼야 할 뒷높이가 표와 똑같이 딱 떨어진다 · §N-그림 */
  if (자리.그림 != null) {
    // 짧은 꼴(경로 문자열)은 높이가 없으니 채움이다
    const g = typeof 자리.그림 === 'string' ? {} : (자리.그림 ?? {});
    const 채움 = g.높이 == null || g.높이 === '채움';
    if (채움) {
      const 막는것 = ['문단', '목록', '번호목록', '표', '단계띠', '수치']
        .filter((k) => 자리[k] != null);
      if (막는것.length) throw new Error(
        `자리 ${i} 의 그림이 남은 높이를 다 먹는데(높이 "채움") 앞에 ${막는것.join(' · ')} 가 있다. ` +
        `줄 수를 미리 못 세서 남는 높이를 못 잰다. 그림 "높이" 를 블록 수로 주거나 제목 · 요약만 둔다`);
    }
    const 앞 = (자리.제목 ? 42 : 0) + (자리.요약 ? 42 : 0) +
      (자리.제목 || 자리.요약 ? 42 : 0);          // 머리뒤 한 칸
    o.push(그림그리기(자리, i, r.h - pad * 2, 앞, 자리.출처 ? 42 : 0));
  }
