/* 자리재기 — **덩이가 자리 안에 앉는가**를 진짜 브라우저로 잰다.
 *
 *   node scripts/자리재기.mjs content/_check/표.json
 *
 * `기준선.mjs` 는 덩이의 top 과 높이가 42 배수인지 본다. 그것만으로는
 * **덩이가 자리 밖으로 넘치는지 · 계산한 자리에 실제로 앉았는지**를 못 잡는다 —
 * 84 도 126 도 다 42 배수라서 42px 어긋난 표가 그대로 통과했다(실측 · N-배경 b6).
 *
 * 그래서 여기서는 자리(.bx)의 **안쪽 원점**을 기준으로 잰다.
 *   ① 덩이 top 이 42 배수인가
 *   ② 마지막 덩이 아래가 자리 안쪽을 넘지 않는가
 *
...
if (!src) {
  console.error('쓰기 : node scripts/자리재기.mjs <문안.json>');
  process.exit(1);
}
...
const tmp = path.join(root, 'out/html/_자리재기.html');
...
const 잰것 = await p.evaluate(() => {
  ...
  document.querySelectorAll('.sheet').forEach((sh, 면) => {
    sh.querySelectorAll('.bx').forEach((bx, 자리) => {
...
console.log(`\n${src} · 잰 덩이 ${잰것.length}개 · 자리 안쪽 원점 기준\n`);
if (어긋.length) {
  console.log('면 자리 덩이            top   아래남음   왜');
  for (const r of 어긋) {
    const 왜 = [r.top % 42 !== 0 ? 'top 이 42 배수가 아니다' : '', r.아래 < 0 ? '자리를 넘쳤다' : '']
      .filter(Boolean).join(' · ');
    console.log(`${String(r.면).padStart(2)}  ${r.자리}  ${r.덩이.padEnd(14)}${String(r.top).padStart(5)}${String(r.아래).padStart(9)}   ${왜}`);
  }
}
