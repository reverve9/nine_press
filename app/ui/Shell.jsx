...
/* **얹은 것은 고른 박스 자리에 놓인다** · 사용자 판정.
   갈래마다 다른 자리를 기본으로 두었더니 박스를 골라 둔 채 「+ 글」을 눌렀는데
   글이 카피 밴드에 생기는 함정이 됐다 — 고른 것과 놓이는 자리가 어긋났다.

   **놓기와 얹기는 대등하다.** 둘 다 「내용을 하나 더 놓는 일」이고 ·
   다른 것은 자리를 누가 정하느냐 하나다 — 놓기는 흐름이 정하고 · 얹기는 내가 정한다.
   그러니 얹기도 「고른 박스 안」에서 시작하는 것이 맞다. 판 절대 좌표라는 것은
   그대로다 · 놓은 뒤에는 어디로든 끌어갈 수 있다.

   박스를 안 골랐으면 박스 존 전체(프레임 폭 · 존 맨 위)를 자리로 삼는다. */
const 새얹기 = {
  선: (자리) => ({ 선: '가로', x: 자리.x, y: 자리.y, 길이: 자리.폭, 굵기: 2, 색: '선' }),
  도형: (자리) => ({ 도형: { 배경: '블록배경', 모서리: 10 },
    x: 자리.x, y: 자리.y, 폭: 자리.폭, 높이: 자리.높이 }),
...
/* ── 도형 판 · 한 부품 · N-도크재편 ─────────────────────
   **도형 일곱이 세 곳에 있었다** · 박스 · 요소 · 얹기. 어휘가 같고 규칙이 같은데
   판이 셋으로 갈려 있어 한 곳을 고치면 나머지 둘이 뒤처졌다 — 실제로 갈려 있었다 ·
   박스에만 테두리 최근색이 있고 요소 · 얹기에는 없었다. 한 부품으로 묶는다.

   다른 것은 걸리는 범위와 「글자」 하나뿐이다.
   **얹기에는 「글자」를 안 낸다** · 렌더러가 `data-글자` 를 얹은 것에 안 붙인다 —
   내면 문안에만 남고 판에 안 나오는 죽은 열쇠가 된다 · 렌더러를 고칠 일이라 남긴다 */
function 도형판({ 도형: s = {}, 놓기, 최근색, 색기억, 로그, 글자 = false }) {
...
export default function Shell({ docs, first }) {
  ...
  /* 오른쪽 도크 · 지금 연 탭 · **둘뿐이다** · N-도크재편.
     탭 넷(판면 · 박스 · 요소 · 얹기)이 두 무리를 섞고 있었다 · 사용자 판정.
     판면과 박스는 **그릇**이다 — 박스가 몇 개 생기나 · 그 박스가 어떻게 생겼나.
     놓기와 얹기는 **내용**이다 — 무엇을 담나. 그릇 둘 · 내용 둘로 갈라 탭 둘이 된다 */
  const [탭, set탭] = useState('판');
  /* 내용을 어디에 놓나 · **놓기와 얹기의 유일한 차이다** · 사용자 판정.
     놓기는 자리를 흐름이 정하고(순서대로 쌓인다) · 얹기는 내가 정한다(판 절대 좌표).
     그 하나뿐인 차이를 스위치 하나로 드러낸다 — 갈래 목록도 · 고른 것 판도 이걸 따른다.
  ...
  /* 얹을 자리 — 고른 박스의 **안쪽** 사각형이다. 안여백을 뺀 값이라 여기 놓은 것이
     박스 안 글과 왼쪽 · 첫 줄이 정확히 맞는다(안쪽 원점이 이미 42 기준선에 앉아 있다).
     박스를 안 골랐으면 박스 존 전체를 준다 · 판 밖으로는 안 나간다. */
  const 얹을자리 = () => {
    const 모드 = 현재?.모드 === '연속' ? '연속' : '카피';
    const 존 = { x: _규격.프레임.x, y: _규격.존[모드], 폭: _규격.프레임.w,
      높이: _규격.존높이(모드), 박스: false };
    if (박스번호 == null || !현재) return 존;
    try {
      const r = 영역(현재)[박스번호];
      if (!r) return 존;
      const pad = 현재.박스?.[박스번호]?.안여백 ?? doc?.안여백 ?? _규격.안여백기본;
      return { x: r.x + pad, y: r.y + pad, 폭: r.w - pad * 2, 높이: r.h - pad * 2, 박스: true };
    } catch { return 존; }
  };
  ...
  /* 층 목록에서 오르내린다 · **데이터는 안 바꾼다** · N-도크재편 · 사용자 판정.
     내용이 대등해져 겹칠 수 있게 되니 무엇이 위인지 볼 자리가 들었다. 그 자리를
     한 목록으로 내되 **문안은 지금 그대로다** — `층: '앞'|'뒤'` 와 배열 순서 둘뿐이다.
     숫자 층을 새로 만들면 렌더러 계약이 바뀌어 봉인본 회귀가 깨진다.

     쌓임은 DOM 순서가 전부다 · 같은 층에서는 **배열 뒤가 위**다.
     그래서 목록은 배열을 뒤집어 보여 주고 · 「위로」는 배열 뒤로 간다.
     제 층의 끝에서 한 걸음 더 가면 **박스 칸막이를 넘어** 층이 앞 ↔ 뒤로 바뀐다 ·
     넘어간 쪽에서는 칸막이에 가장 가까운 자리에 선다 — 눈에 보이는 대로다. */
  const 층옮기기 = (k, 걸음) => 바꾸기((d) => {
    const a = 얹기들(d);
    const o = a?.[k];
    if (!o) return false;
    const 층 = (x) => ((x.층 ?? '뒤') === '앞' ? '앞' : '뒤');
    const 내층 = 층(o);
    const 같은층 = a.map((_, j) => j).filter((j) => 층(a[j]) === 내층);
    const 자리 = 같은층.indexOf(k);
    const 이웃 = 걸음 < 0 ? 같은층[자리 + 1] : 같은층[자리 - 1];
    if (이웃 != null) {
      [a[k], a[이웃]] = [a[이웃], a[k]];
      set얹기번호(이웃);
      return;
    ...
    if (걸음 < 0 && 내층 === '뒤') {
      a.splice(k, 1);
      o.층 = '앞';
      a.unshift(o);          // 앞 무리의 맨 아래 · 칸막이 바로 위다
      set얹기번호(0);
      return;
    }
    if (걸음 > 0 && 내층 === '앞') {
      a.splice(k, 1);
      delete o.층;           // 「뒤」가 기본이다 · 기본값을 문안에 안 남긴다
      a.push(o);             // 뒤 무리의 맨 위 · 칸막이 바로 아래다
      set얹기번호(a.length - 1);
      return;
    }
  ...
  const 얹기그림놓기 = async (경로) => {
    ...
    바꾸기((d) => {
      ...
      const 자 = 얹을자리();
      const 새 = { 그림: 경로, x: 옛?.x ?? 자.x, y: 옛?.y ?? 자.y, 폭, 높이,
        ...(옛?.층 ? { 층: 옛.층 } : {}) };
  ...
  const 얹기갈래바꾸기 = (갈래) => {
    ...
    바꾸기((d) => {
      ...
      // 갈아탈 때는 있던 자리를 지킨다 · 눈앞에서 물건이 순간이동하지 않는다
      a[얹기번호] = { ...새얹기[갈래](얹을자리()), x: o.x, y: o.y,
        ...(o.층 ? { 층: o.층 } : {}) };
    }, { 그리기: true });
  };
...
>폰트</button>
      ...
      <aside className="dock">
        ...
        {/* 탭 둘 · **그릇과 내용이다** · 사용자 판정 · N-도크재편.
            ...
            놓기와 얹기는 「무엇을 담나」다. 층이 다른 넷이 한 줄에 나란히 서서
            지금 그릇을 만지는지 내용을 만지는지가 안 읽혔다.
            **「표」 · 「그림」이 요소와 나란히 설 물건이 아닌 것과 같은 이유다** · N-자유 c */}
        <div className="tabs">
          {['판', '내용'].map((v) => (
        ...
        <div className="dkbd">
          ...
          {/* ── 내용 · 층 목록 ───────────────────────────
              **놓기와 얹기는 대등하다** · 사용자 판정. 둘 다 「내용을 하나 더 놓는 일」이고
              ...
              얹기는 내가 정한다(판 절대 좌표). 대등해졌으니 **겹칠 수 있고** ·
              그러면 무엇이 위인지 볼 자리가 있어야 한다.

              **데이터는 안 바꿨다** · `층: '앞'|'뒤'` + 배열 순서 그대로다 · UI 만 한 목록이다.
              박스와 그 안 요소는 통째로 칸막이 한 칸에 든다 — 렌더러가 얹기층('뒤') ·
                ...
                <div className={'lyrbx' + (삽입처 === '박스' ? ' on' : '')}
                  ...
                  {고른 && (
                    <em>{박스번호 + 1} / {현재?.박스?.length}
                      {고른.비움 ? ' · 비움' : 고른내용 ? ` · ${고른내용.length}개${옛꼴 ? ' · 옛 꼴' : ''}` : ''}</em>
                  )}
                </div>
                {고른 && !고른.비움 && 고른내용 && (
                  <div className="elrows lyrin">
                    {고른내용.map((el, j) => {
                      const k = 요소열쇠(el);
                      return (
...
>민글</button>
          ...
          {탭 === '내용' && 삽입처 === '박스' && !!고른요소 && !고른?.비움 && (
            /* 요소 도형 — 박스 도형과 **같은 어휘 · 같은 규칙**이다.
               ...
               **판의 꼬리다** · 갈래와 무관하게 언제나 맨 아래에 있다.
               못 고를 때 말은 위 층 목록이 이미 한다 — 여기서 또 하지 않는다 */
            <>
              <div className="popln" />
                  ...
                  <>
                    ...
                    {k === '도형' && (
                      <>
                        ...
                        {/* 「글자」를 안 낸다 — 렌더러가 얹은 것에 `data-글자` 를 안 붙여
                            문안에만 남고 판에 안 나오는 죽은 열쇠가 된다 · N-도크재편 */}
                        <도형판 도형={o.도형} 놓기={얹기도형}
                                최근색={최근색} 색기억={색기억} 로그={set로그} />
                      </>
                    )}
