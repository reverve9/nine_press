...
/* ── 값 갈래 넷 · 화면 문법을 여기에 맞춘다 ────────────────
   ...
   진짜 계약은 렌더러가 지킨다. */
const 수범위 = { 모서리: [0, 40], 투명도: [0, 100], 굵기: [1, 6], 폭: [1, 표열최대] };
...
export default function Shell({ docs, first }) {
  ...
  const 표열넣기 = () => 표바꾸기((t) => {
    if (표열수(t) >= 표열최대) { set로그(`열은 ${표열최대} 까지다`); return false; }
    if (t.머리) t.머리.push('');
  ...
  /* 폭 — 균등 트랙을 몇 개 먹느냐다. 하나라도 손대면 열 수만큼 전부 적는다.
     렌더러가 「열 수와 같은 길이의 배열」을 요구한다 */
  const 표폭 = (c, n) => 표바꾸기((t) => {
  ...
  const 표균등 = () => 표바꾸기((t) => { if (!t.폭) return false; delete t.폭; });
              ...
              <팔레트 이름="표" 열쇠="표" 차례={1}
                ...
                {!켬 ? (
                  ...
                  <>
                    ...
                    <줄 이름="폭" 곁={t.폭 ? `${t.폭.join(' : ')} · 1~${표열최대}` : '균등'}>
                      {Array.from({ length: n }, (_, c) => (
                        <수칸 key={c} 열쇠="폭" 값={t.폭?.[c]} 기본={1} 좁게
                              로그={set로그} 놓기={(v) => 표폭(c, v)} />
                      ))}
                      <button className="chip" onClick={표균등} disabled={!t.폭}
                              title="폭을 지우면 열이 균등하게 나뉜다">균등</button>
                    </줄>
                    ...
                    <줄 이름="채움" 곁="남는 높이를 42 걸음으로 나눈다">
                      <button className={'chip' + (t.채움 ? ' on' : '')}
                              onClick={() => 표채움(!t.채움)}
                              title="자리에 남은 높이를 칸들이 나눠 갖는다 · 앞에 제목 · 요약만 있을 때">
                        채움
                      </button>
                    </줄>
                    <줄 이름="표" 곁="이 자리에서 없앤다">
