...
function 구조칸들(면, 표적) {
  ...
  (b.내용 ?? []).forEach((it, ii) => {
    ...
    /* 표는 여기서 안 다룬다 · N-배경 b2.
       ...
       새 렌더러는 `행:[[…]]` 만 읽고 · 열 · 선 · 칠 · 높이는 「표」 팔레트가 고친다. */
  });
  return out;
}
      ...
      <div className="stage">
        ...
        {/* 떠 있는 속성 패널 — 어도비 팔레트 짜임 · 머리를 잡아 끌고 · 두 번 누르면 접힌다.
          ...
          {패널.표 && (() => {
            ...
            // 높이 · 세로 — 갈래 · 공백 자리 · 슬롯 읽기
            const 높이갈 = t ? 높이갈래(t.높이) : '끔';
              ...
              <팔레트 이름="표" 열쇠="표" 차례={1}
                ...
                {!켬 ? (
                  ...
                  <>
                    ...
                    {/* 높이 · 세로 · N-배경 b5 · b7 — **가로(폭)와 한 쌍이다.**
                        세로는 42 격자에 갇혀 한 칸이 4.76%p 다. 정확히 나누려면 몫을 쓴다 */}
                    <줄 이름="높이"
                        곁={높이갈 === '줄' ? '칸마다 한 줄 42'
                          : 높이갈 === '채움' ? '자리를 고르게 채운다 · 남음은 위아래로'
                          : 슬롯보기.join(' · ')}>
                      <span className="seg">
                        {높이갈래들.map(([v, 이름]) => (
                          <button key={v} className={'chip' + (높이갈 === v ? ' on' : '')}
                                  onClick={() => 표높이갈래(v)}
                                  title={v === '줄' ? '칸마다 한 줄 42 · 자리를 안 채운다'
                                    : v === '채움' ? '남은 높이를 칸들이 고르게 나눈다'
