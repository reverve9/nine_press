...
/* ── 고친 DOM 을 원문 표기로 되돌린다 ──
   판면에서 그 자리에 타이핑하면 결과는 HTML 이다. 그걸 그대로 저장하면
   문안의 원본이 HTML 이 되어 버린다. 다시 `**굵게**` · {TBD} · {→05} 로 되돌린다. */
function 원문(node) {
  let s = '';
...
export default function Shell({ docs, first }) {
  ...
  async function 저장() {
    ...
    if (r.ok) { setMtime(r.mtime); set더러움(false); set로그('저장됨'); set판본키((n) => n + 1); }  // 저장 뒤 한 번만 다시 그린다
    else set로그(r.사유);
    set바쁨(false);
  }
  ...
  function 재기() {
    ...
    // ① 판면에서 글자를 그 자리에 고친다.  블록 빈 곳을 누르면 폼으로 간다
    try {
      ...
      if (d && !d.body.dataset.집음) {
        ...
        st.textContent =
          ...
          '[data-p][contenteditable="true"]{background:#fff;outline:calc(2*var(--u)) solid #E68100;' +
        ...
        d.addEventListener('mousedown', (e) => {
          ...
          t.dataset.전 = 원문(t);
          t.dataset.전html = t.innerHTML;   // Escape 로 되돌릴 자리
        ...
        d.addEventListener('focusout', (e) => {
          ...
          const 뒤 = 원문(t);
          if (뒤 !== t.dataset.전) 제자리고침(JSON.parse(t.dataset.p), 뒤);
        }, true);
        ...
        // 글자가 아닌 곳을 누르면 그 블록 폼으로 — 줄 추가·삭제는 거기서
