...
// 좌표(`행-열-블록`)를 data-b 로 심는다. 도구가 판면을 클릭했을 때 어느 블록인지 아는 통로다.
// 산출 HTML 에도 같이 들어가지만 판면에는 아무 영향이 없다.
function renderBlock(b, { 채움, 폭 = 1, 좌표 }) {
  const 배경 = b.배경 ? ` ${b.배경}` : '';
  const db = 좌표 ? ` data-b="${좌표}"` : '';
  // --w 는 3열 균등 고정 규칙(.row.g)이 폭을 읽는 통로다. flex 만으로는 폭이 지워진다.
  const flex = (b.채움 ?? 채움)
    ? ` style="flex:${폭};--w:${폭}"`
    : ` style="--w:${폭}"`;
  const o = [`<div class="b${배경}"${flex}${dk(b.이름)}${db}>`];
  // 블록 머리는 라벨 하나다 (§7 판정 ⑤ — .bt 폐기).
  // 옛 문안의 `제목` 은 라벨로 받아 준다.
  const 머리 = b.라벨 ?? b.제목;
  if (머리) o.push(`<div class="bl">${inline(머리)}</div>`);
...
function renderCol(col, ci = 0, ri = 0) {
  ...
  if (blocks.length === 1) {
    return renderBlock(blocks[0], { 채움: true, 폭, 좌표: `${ri}-${ci}-0` });
  }

  const inner = blocks
    .map((b, i) => renderBlock(b, { 채움: i === blocks.length - 1, 좌표: `${ri}-${ci}-${i}` }))
    .join('\n');
...
function renderFoot(항목) {
  if (!항목?.length) return '';
  return `<div class="foot" data-b="head">
  <div class="pt">
